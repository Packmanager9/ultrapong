...
//         console.log(gamepadAPI.axesStatus[1]*gamepadAPI.axesStatus[0]) //debugging
    ...
    class Pongball{
        constructor(){
            this.body = new Circle(350,350, 320, "white")
            let angle = Math.PI*2*Math.random()
            this.ball = new Circle(350,350, 5, "#00ff00", (Math.cos(angle))*8,  (Math.sin(angle))*8)
            this.cneter = new Circle(350,350, 3, "red")
            this.angle = 0
            this.dis = 310
            this.width = Math.PI/8
            this.arc = new Shape([])
            this.score = 0
        }
        draw(){
            ...
            if(typeof (gamepadAPI.axesStatus[1]) != 'undefined'){
                if(typeof (gamepadAPI.axesStatus[0]) != 'undefined'){
                   this.angle = (new Line( gamepadAPI.axesStatus[0], gamepadAPI.axesStatus[1],0,0)).angle()
                }else{
                    this.angle = (new LineOP(TIP_engine,this.body)).angle()
                }
            }else{
                this.angle = (new LineOP(TIP_engine,this.body)).angle()
            }
            ...
            canvas_context.beginPath()
            canvas_context.strokeColor = "red"
            canvas_context.arc(this.body.x, this.body.y, this.dis, this.angle-this.width,  this.angle+this.width, false)
            canvas_context.stroke()


            let shape = []
            let angle = this.angle-this.width
            for(let t = 0;t<100;t++){
                shape.push(new Circle(this.body.x+(Math.cos(angle)*this.dis), this.body.y+(Math.sin(angle)*this.dis),5, "yellow" ))
                angle += (this.width)/50
            }


            this.arc = new Shape(shape)
            ...
            if(this.arc.metaDoesPerimeterTouch(this.ball) != false){
                this.score++
                let vec =new Vector(this.ball, this.ball.xmom*1.02, this.ball.ymom*1.02)
                // let link = new LineOP(this.body, this.arc.metaDoesPerimeterTouch(this.ball))
                let link = new LineOP(this.body, this.arc.shapes[50])
                let angle = link.angle() 
                vec.rotate(angle)
                this.ball.xmom = vec.xmom
                this.ball.ymom = vec.ymom
            }
            this.ball.draw()
            this.body.draw()
            this.cneter.draw()
            ...
            canvas_context.fillText(this.score, 20,20)
        }
    }
